...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (loginType === "admin") {
        ...
        // Let middleware handles the redirect to /dashboard
  ...
  return (
    <div className="flex min-h-screen items-center justify-center px-4" style={{ background: "var(--background)" }}>
      <div className="w-full max-w-sm rounded-[2.5rem] bg-white p-10 shadow-2xl shadow-blue-100/50">
        <header className="text-center">
          ...
          <h1 className="font-display text-4xl font-black text-slate-800">
            Welcome back
          </h1>
        ...
        {/* Login Type Tabs */}
        <div className="mt-10 flex rounded-2xl bg-slate-50 p-1.5 border border-slate-100">
          <button
            onClick={() => {
                setLoginType("admin");
                setError("");
            ...
            className={`flex-1 rounded-xl py-3 text-xs font-black uppercase tracking-tight transition-all ${
              loginType === "admin" ? "bg-white text-blue-600 shadow-sm border border-slate-100" : "text-slate-400 hover:text-slate-600"
            }`}
          >
            Admin / HR
          </button>
          <button
            ...
            className={`flex-1 rounded-xl py-3 text-xs font-black uppercase tracking-tight transition-all ${
              loginType === "employee" ? "bg-white text-blue-600 shadow-sm border border-slate-100" : "text-slate-400 hover:text-slate-600"
            }`}
          >
            Employee
          </button>
        ...
        <form onSubmit={handleSubmit} className="mt-8 space-y-5">
          ...
          <div className="space-y-2">
            ...
            {loginType === "employee" && !password && (
                <p className="text-[10px] font-bold text-slate-400 px-1">
                  Default: <span className="text-blue-500">AskTech@123</span>
                </p>
            )}
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="mt-6 w-full rounded-2xl bg-blue-600 py-4.5 text-sm font-black text-white shadow-xl shadow-blue-100 transition-all hover:bg-blue-700 hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50"
            style={{ minHeight: 54 }}
          >
